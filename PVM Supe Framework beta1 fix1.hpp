...
public:
    ...
    void step() {
        ...
        case 0x45: { 
            uint32_t r = read32(); 
            if (cells[r].type == T_STR) { 
                char* endptr;
                // Convert string to long long
                long long val = std::strtoll(cells[r].v_str.c_str(), &endptr, 10);
                
                // If endptr points to the start, or conversion failed completely
                if (endptr == cells[r].v_str.c_str()) {
                    cells[r].v_int = 0;
                } else {
                    cells[r].v_int = val;
                }
